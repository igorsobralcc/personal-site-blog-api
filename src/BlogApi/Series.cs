...
public static partial class SeriesEndpoints
{
    public static void MapSeriesEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        var admin = app.MapGroup("/api/v1/admin/series");
        admin.MapGet("/", List);
    ...
    {
        var (p, s, d, e) = Paging.Read(c);
        if (e is not null)
        {
            return e;
        }

        var all = store.Series.Values.Where(x => d || x.DeletedAt is null).OrderByDescending(x => x.CreatedAt).ToArray();
        return Results.Ok(new Page<object>(all.Skip((p - 1) * s).Take(s).Select(View).ToArray(), p, s, all.Length, (int)Math.Ceiling(all.Length / (double)s)));
    }
    ...
    {
        return store.Series.TryGetValue(id, out var x) ? Results.Ok(x.Revisions) : Problems.Result(c, 404, "Not Found");
    }

    private static IResult Revision(Guid id, int number, HttpContext c, BlogStore store)
    {
        return store.Series.TryGetValue(id, out var x) && x.Revisions.FirstOrDefault(r => r.RevisionNumber == number) is { } r ? Results.Ok(r) : Problems.Result(c, 404, "Not Found");
    }

    private static IResult PublicGet(string slug, HttpContext c, BlogStore store)
    {
        var x = store.Series.Values.SingleOrDefault(y => y.DeletedAt is null && y.Status == PublicationStatus.Published && y.Slug == slug);
        if (x is null)
        {
            return Problems.Result(c, 404, "Not Found");
        }
    ...
    private static object View(ArticleSeries x)
    {
        return new
        {
            x.Id,
    ...
    private static Dictionary<string, string[]>? Validate(ArticleSeries x, BlogStore store, Guid? self)
    {
        var e = new Dictionary<string, string[]>();
        if (x.Slug is { } s && (s.Length > 160 || !Slug().IsMatch(s)))
        {
            e["slug"] = ["Slug is invalid."];
        }

        if (x.Title is { Length: > 200 })
        {
            e["title"] = ["Title is too long."];
        }

        if (x.Summary is { Length: > 500 })
        {
            e["summary"] = ["Summary is too long."];
        }

        if (x.ArticleIds.Distinct().Count() != x.ArticleIds.Count || x.ArticleIds.Any(id => !store.Articles.TryGetValue(id, out var a) || a.DeletedAt is not null))
        {
            e["articleIds"] = ["Article IDs must be unique and active."];
        }

        if (x.Slug is not null && store.Series.Values.Any(y => y.Id != self && y.DeletedAt is null && string.Equals(y.Slug, x.Slug, StringComparison.OrdinalIgnoreCase)))
        {
    ...
    private static void Apply(ArticleSeries x, JsonObject p)
    {
        if (p.ContainsKey("slug"))
        {
            x.Slug = Clean(p["slug"]?.GetValue<string>());
        }

        if (p.ContainsKey("title"))
        {
            x.Title = Clean(p["title"]?.GetValue<string>());
        }

        if (p.ContainsKey("summary"))
        {
            x.Summary = Clean(p["summary"]?.GetValue<string>());
        }

        if (p["status"] is JsonValue status)
        {
            x.Status = Enum.Parse<PublicationStatus>(status.GetValue<string>(), true);
        }

        if (p.ContainsKey("articleIds"))
        {
            x.ArticleIds = p["articleIds"]?.AsArray().Select(v => v!.GetValue<Guid>()).ToList() ?? [];
        }
    }
    private static bool Allowed(PublicationStatus from, PublicationStatus to)
    {
        return from == to || (from, to) is (PublicationStatus.Writing, PublicationStatus.Draft) or (PublicationStatus.Draft, PublicationStatus.Writing or PublicationStatus.Published or PublicationStatus.NotListed) or (PublicationStatus.Published, PublicationStatus.NotListed or PublicationStatus.Archived) or (PublicationStatus.NotListed, PublicationStatus.Draft or PublicationStatus.Published or PublicationStatus.Archived) or (PublicationStatus.Archived, PublicationStatus.Draft);
    }

    private static ArticleSeries Clone(ArticleSeries x)
    {
        return new()
        {
            Id = x.Id,
            ...
            ArticleIds = [.. x.ArticleIds]
        };
    }

    private static void Copy(ArticleSeries f, ArticleSeries t)
    {
        t.Slug = f.Slug;
        t.Title = f.Title;
        t.Summary = f.Summary;
    ...
    {
        x.Revisions.Add(new(x.Revisions.Count + 1, operation, at, "site-owner", c.TraceIdentifier, JsonSerializer.SerializeToNode(View(x))!.AsObject()));
    }
