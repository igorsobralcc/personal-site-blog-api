...
public sealed class AdminKeyMiddleware(RequestDelegate next, IConfiguration configuration)
{
    ...
    {
        if (!context.Request.Path.StartsWithSegments("/api/v1/admin")) { await next(context); return; }
        ...
        { await Problems.Write(context, 401, "Unauthorized", "An administrative credential is required."); return; }
        await next(context);
    }
}
...
public static class HttpConcurrency
{
    ...
    { context.Response.Headers.ETag = etag; return context.Request.Headers.IfNoneMatch.ToString() == etag; }
}
...
public static class Paging
{
    ...
    {
        var pt = context.Request.Headers["X-Page"].ToString(); var st = context.Request.Headers["X-Page-Size"].ToString();
        var page = string.IsNullOrEmpty(pt) ? 1 : int.TryParse(pt, out var p) ? p : 0;
        var size = string.IsNullOrEmpty(st) ? 20 : int.TryParse(st, out var s) ? s : 0;
        var deleted = bool.TryParse(context.Request.Headers["X-Include-Deleted"], out var d) && d;
        if (page < 1 || size is < 1 or > 100) return (page, size, deleted,
