...
{
    Writing, Draft, Published, NotListed, Archived
}
...
public static partial class ArticleEndpoints
{
    public static void MapArticleEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        admin.MapPatch("/{id:guid}", Patch).Accepts<JsonObject>("application/merge-patch+json");
        admin.MapDelete("/{id:guid}", Delete);
        admin.MapPost("/{id:guid}/restore", Restore);
        admin.MapGet("/{id:guid}/revisions", Revisions);
    ...
    {
        return store.Articles.TryGetValue(id, out var a) ? Results.Ok(a.Revisions) : Problems.Result(c, 404, "Not Found");
    }

    private static IResult Revision(Guid id, int number, HttpContext c, BlogStore store)
    {
        return store.Articles.TryGetValue(id, out var a) && a.Revisions.FirstOrDefault(r => r.RevisionNumber == number) is { } r ? Results.Ok(r) : Problems.Result(c, 404, "Not Found");
    }
    ...
    private static IResult PublicFeed(HttpContext c, BlogStore store, IConfiguration config, int limit = 8, string? cursor = null)
    {
        ...
        Guid? beforeId = null;
        if (cursor is not null && !Cursor.TryRead(cursor, config["CursorKey"] ?? config["AdminKey"] ?? "development-only", out beforeTime, out beforeId))
        {
            return Problems.Result(c, 400, "Validation failed", "The cursor is invalid.");
        }
        ...
        if (beforeTime is not null)
        {
            query = query.Where(a => a.PublishedAt < beforeTime || a.PublishedAt == beforeTime && a.Id.CompareTo(beforeId!.Value) < 0);
        }

        var rows = query.Take(limit + 1).ToArray();
        var items = rows.Take(limit).Select(a => Summary(a, store)).ToArray();
        string? next = null;
    ...
    private static IResult PublicDetail(string slug, HttpContext c, BlogStore store, IConfiguration config)
    {
        ...
        var result = new
        {
            ...
            tags = a.Tags,
            a.BodyVersion,
            body = PublicBody(a.Body, store),
            canonicalUrl = $"{(config["PublicSiteOrigin"] ?? "https://example.invalid").TrimEnd('/')}/articles/{a.Slug}",
    ...
    internal static dynamic Summary(Article a, BlogStore store)
    {
        return new
        {
            id = a.Id,
            ...
            topic = a.Topic,
            image = Image(a.EditorialMediaId, null, null, store)
        };
    }

    private static PublicImage? Image(Guid? id, string? alt, string? caption, BlogStore store)
    {
        return id is { } value && store.Media.TryGetValue(value, out var m)
            ? new(m.Url, alt ?? m.Alt, m.Width, m.Height, caption ?? m.Caption) : null;
    }

    private static JsonArray PublicBody(JsonArray body, BlogStore store)
    {
        var clone = body.DeepClone().AsArray();
        foreach (var node in clone.OfType<JsonObject>().Where(x => x["type"]?.GetValue<string>() == "image"))
        {
            var id = node["mediaId"]!.GetValue<Guid>();
            var image = Image(id, node["alt"]?.GetValue<string>(), node["caption"]?.GetValue<string>(), store)!;
            node.Remove("mediaId");
            node["url"] = image.Url;
            node["alt"] = image.Alt;
    ...
    private static object AdminView(Article a)
    {
        return new
        {
            a.Id,
    ...
    private static Dictionary<string, string[]>? Validate(Article a, BlogStore store, Guid? self)
    {
        ...
        if (a.Slug is { } slug && (slug.Length > 160 || !SlugRegex().IsMatch(slug)))
        {
            e["slug"] = ["Slug format is invalid."];
        }

        Limits(a.Title, 200, "title");
        Limits(a.Summary, 500, "summary");
        Limits(a.Topic, 80, "topic");
        Limits(a.SeoTitle, 70, "seoTitle");
        Limits(a.SeoDescription, 180, "seoDescription");
        if (a.Tags.Count > 20 || a.Tags.Any(t => string.IsNullOrWhiteSpace(t) || t.Trim().Length > 40) || a.Tags.Select(t => t.Trim()).Distinct(StringComparer.OrdinalIgnoreCase).Count() != a.Tags.Count)
        {
            e["tags"] = ["Tags are invalid or duplicated."];
        }

        string? bodyError = null;
        if (a.Body.Count > 500 || Encoding.UTF8.GetByteCount(a.Body.ToJsonString()) > 1024 * 1024 || !ValidBody(a.Body, store, out bodyError))
        {
            e["body"] = [bodyError ?? "Body is invalid."];
        }

        if (a.Slug is not null && store.Articles.Values.Any(x => x.Id != self && x.DeletedAt is null && string.Equals(x.Slug, a.Slug, StringComparison.OrdinalIgnoreCase)))
        {
            e["slug"] = ["Slug is already active."];
        }

        foreach (var id in new[] { a.EditorialMediaId, a.SocialMediaId }.Where(x => x is not null).Cast<Guid>())
        {
            if (!store.Media.TryGetValue(id, out var m) || m.DeletedAt is not null)
            {
                e["mediaId"] = ["Media does not exist or is deleted."];
            }
        }

        return e.Count == 0 ? null : e;
        void Limits(string? value, int max, string key)
        {
            if (value is { } v && (v.Length < 1 || v.Length > max))
            {
                e[key] = [$"Must contain 1 through {max} characters."];
            }
        }
    }
    private static bool ValidBody(JsonArray body, BlogStore store, out string? error)
    {
        ...
        {
            if (node is not JsonObject b || b["type"] is not JsonValue typeValue || !typeValue.TryGetValue<string>(out var type))
            {
                error = "Every block requires a type.";
                return false;
            }
            bool Text(string name)
            {
                return b[name] is JsonValue v && v.TryGetValue<string>(out var s) && !string.IsNullOrWhiteSpace(s);
            }
            ...
            switch (type)
            {
                ...
                case "code":
                    ...
                    break;
                case "list":
                    if (b["items"] is not JsonArray items || items.Count == 0 || items.Any(x => x is not JsonValue v || !v.TryGetValue<string>(out var s) || string.IsNullOrWhiteSpace(s)))
                    {
                    ...
                    break;
                case "image":
                    if (b["mediaId"] is not JsonValue idv || !idv.TryGetValue<Guid>(out var id) || !store.Media.TryGetValue(id, out var media) || media.DeletedAt is not null)
                    {
                        error = "Image media is missing or deleted.";
                        return false;
                    }
                    break;
                case "table":
                    if (!Text("caption") || b["headers"] is not JsonArray headers || headers.Count == 0 || b["rows"] is not JsonArray rows || rows.Any(r => r is not JsonArray cells || cells.Count != headers.Count))
                    {
                        error = "Table shape is invalid.";
                        return false;
                    }
                    break;
                default:
    ...
    private static int ReadingTime(JsonArray body)
    {
        ...
        foreach (var b in body.OfType<JsonObject>())
        {
            ...
            if (type == "code")
            {
                codeLines += b["code"]!.GetValue<string>().Split('\n').Count(x => !string.IsNullOrWhiteSpace(x));
                continue;
            }
            foreach (var key in new[] { "text", "caption" })
            {
                if (b[key] is JsonValue v && v.TryGetValue<string>(out var s))
                {
                    prose.Append(' ').Append(s);
                }
            }

            foreach (var key in new[] { "items", "headers" })
            {
                if (b[key] is JsonArray values)
                {
                    foreach (var v in values)
                    {
                        prose.Append(' ').Append(v?.GetValue<string>());
                    }
                }
            }

            if (b["rows"] is JsonArray rows)
            {
                foreach (var row in rows.OfType<JsonArray>())
                {
                    foreach (var cell in row)
                    {
                        prose.Append(' ').Append(cell?.GetValue<string>());
                    }
                }
            }
        }
        var words = WordRegex().Matches(prose.ToString()).Count;
        return Math.Max(1, (int)Math.Ceiling(words / 200d + codeLines / 12d));
    }
    private static HashSet<Guid> CollectMedia(Article a)
    {
        var result = new HashSet<Guid>();
        if (a.EditorialMediaId is { } e)
        {
            result.Add(e);
        }

        if (a.SocialMediaId is { } s)
        {
            result.Add(s);
        }

        foreach (var b in a.Body.OfType<JsonObject>().Where(x => x["type"]?.GetValue<string>() == "image"))
        {
            result.Add(b["mediaId"]!.GetValue<Guid>());
        }
    ...
    private static void Apply(Article a, JsonObject p)
    {
        string? S(string key, string? old)
        {
            ...
            return p[key]?.GetValue<string>() is { } value ? Clean(value) : null;
        }
        a.Slug = S("slug", a.Slug);
        a.Title = S("title", a.Title);
        a.Summary = S("summary", a.Summary);
        a.Topic = S("topic", a.Topic);
        a.SeoTitle = S("seoTitle", a.SeoTitle);
        a.SeoDescription = S("seoDescription", a.SeoDescription);
        if (p["status"] is JsonValue sv)
        {
            a.Status = Enum.Parse<PublicationStatus>(sv.GetValue<string>(), true);
        }

        if (p.ContainsKey("body"))
        {
            a.Body = p["body"]?.DeepClone().AsArray() ?? [];
        }

        if (p.ContainsKey("tags"))
        {
            a.Tags = p["tags"]?.AsArray().Select(x => x!.GetValue<string>().Trim()).ToList() ?? [];
        }

        if (p.ContainsKey("editorialMediaId"))
        {
            a.EditorialMediaId = p["editorialMediaId"]?.GetValue<Guid>();
        }

        if (p.ContainsKey("socialMediaId"))
        {
            a.SocialMediaId = p["socialMediaId"]?.GetValue<Guid>();
        }
    }
    private static bool Allowed(PublicationStatus from, PublicationStatus to)
    {
        return from == to || (from, to) is
    ...
    private static Article Clone(Article a)
    {
        return new()
        {
            Id = a.Id,
    ...
    private static void Copy(Article from, Article to)
    {
        to.Slug = from.Slug;
        to.Title = from.Title;
        ...
        to.SeoDescription = from.SeoDescription;
        to.Status = from.Status;
        to.PublishedAt = from.PublishedAt;
        to.Body = from.Body;
        to.Tags = from.Tags;
        to.EditorialMediaId = from.EditorialMediaId;
        to.SocialMediaId = from.SocialMediaId;
    }
    private static bool Equivalent(Article a, Article b)
    {
        return JsonSerializer.Serialize(AdminView(a)) == JsonSerializer.Serialize(AdminView(b));
    }

    private static void AddRevision(Article a, string operation, HttpContext c, DateTimeOffset at)
    {
        a.Revisions.Add(new(a.Revisions.Count + 1, operation, at, "site-owner", c.TraceIdentifier, JsonSerializer.SerializeToNode(AdminView(a))!.AsObject()));
    }
...
{
    public static string Write(DateTimeOffset time, Guid id, string key)
    {
        var data = $"1|{time:O}|{id}";
        var sig = Convert.ToHexString(HMACSHA256.HashData(Encoding.UTF8.GetBytes(key), Encoding.UTF8.GetBytes(data))).ToLowerInvariant();
        return Base64Url(Encoding.UTF8.GetBytes($"{data}|{sig}"));
    }
    public static bool TryRead(string cursor, string key, out DateTimeOffset? time, out Guid? id)
    {
        ...
        try
        {
            var value = Encoding.UTF8.GetString(Convert.FromBase64String(cursor.Replace('-', '+').Replace('_', '/').PadRight((cursor.Length + 3) / 4 * 4, '=')));
            var p = value.Split('|');
            if (p.Length != 4 || p[0] != "1" || !DateTimeOffset.TryParseExact(p[1], "O", System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.RoundtripKind, out var t) || !Guid.TryParse(p[2], out var g))
            {
                return false;
            }

            var data = string.Join('|', p[..3]);
            var expected = HMACSHA256.HashData(Encoding.UTF8.GetBytes(key), Encoding.UTF8.GetBytes(data));
            if (!CryptographicOperations.FixedTimeEquals(expected, Convert.FromHexString(p[3])))
            {
                return false;
            }
